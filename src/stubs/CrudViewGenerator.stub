...
class CrudViewGenerator extends Command
{
    ...
    public function generateForm($columns)
    {
        ...
        foreach ($columns as $name => $column) {
            ...
            switch ($column['input']) {
                case 'text':
                    $form .= '<x-inputs.text col="col-12 col-lg-6" name="' . $name . '" label="global.' . $name . '" '.$idAttribute.' value="{{old(\''.$name.'\', $'.$this->modelCamelCase.'->'.$name.' ?? \'\')}}" />' . PHP_EOL . PHP_EOL;
                    break;
                case 'email':
                    $form .= '<x-inputs.email col="col-12 col-lg-6" name="' . $name . '" '.$id.' label="global.' . $name . '" '.$idAttribute.' value="{{old(\''.$name.'\', $'.$this->modelCamelCase.'->'.$name.' ?? \'\')}}" />' . PHP_EOL . PHP_EOL;
                    break;
                case 'date':
                    $form .= '<x-inputs.date col="col-12 col-lg-6" '.$idAttribute.' label="global.'.$name.'" name="'.$name.'" value="{{old(\''.$name.'\', $'.$this->modelCamelCase.'->'.$name.' ?? \'\')}}" />' . PHP_EOL . PHP_EOL;
                    break;
                case 'time':
                    $form .= '<x-inputs.time col="col-12 col-lg-6" '.$idAttribute.' label="global.'.$name.'" name="'.$name.'" value="{{old(\''.$name.'\', $'.$this->modelCamelCase.'->'.$name.' ?? \'\')}}" />' . PHP_EOL . PHP_EOL;
                    break;
                case 'textarea':
                    $form .= '<x-textareas.textarea col="col-12 col-lg-6" rows="6" label="global.'.$name.'" name="'.$name.'" value="{{old(\''.$name.'\', $'.$this->modelCamelCase.'->'.$name.' ?? \'\')}}" />' . PHP_EOL . PHP_EOL;
                    break;
            }
        }
